...
Refactored with validation on submit, UX improvements, NPPES lookup
"""
...
def render_cms1500_form() -> dict:
    """Render complete CMS-1500 form and return data dict on submit, None otherwise"""
    ...
    with st.form("cms_1500_form", clear_on_submit=False):
        ...
        with col1:
            subscriber_name = st.text_input("Subscriber Name *", key="sub_name", placeholder="Last, First")
        ...
        with col2:
            ...
            subscriber_id = st.text_input("Subscriber ID *", key="sub_id", placeholder="Member ID")
        ...
        if auto_accident or other_accident:
            accident_state = st.text_input("State", key="accident_state", placeholder="IL")
        else:
            accident_state = None
        ...
        auth_number = st.text_input("Authorization Number (Optional)", key="auth_number", placeholder="AUTH-123456")
        
        st.divider()
        
        # ===== PROVIDER INFORMATION WITH NPPES LOOKUP =====
        st.subheader("Provider Information (Boxes 24j-33)")
        
        col1, col2 = st.columns(2)
        with col1:
            provider_npi = st.text_input("Provider NPI *", key="provider_npi", placeholder="1234567890")
            
            # NPPES Lookup Button
            if st.button("🔍 Look Up Provider by NPI", key="nppes_lookup_btn"):
                if provider_npi and len(provider_npi) == 10:
                    with st.spinner("Searching NPPES database..."):
                        nppes = get_nppes_lookup()
                        lookup_result = nppes.lookup_npi(provider_npi)
                        if lookup_result:
                            st.success("Provider found!")
                            st.session_state.nppes_result = lookup_result
                        else:
                            st.warning("Provider not found in NPPES. Please enter manually.")
                            st.session_state.nppes_result = None
                else:
                    st.warning("Please enter a valid 10-digit NPI")
        
        with col2:
            provider_tax_id = st.text_input("Provider Tax ID (EIN/SSN) *", key="provider_tax_id", placeholder="12-3456789")
        
        # Check if NPPES result is available in session
        nppes_result = st.session_state.get('nppes_result', None)
        
        col1, col2 = st.columns(2)
        with col1:
            default_first = nppes_result.get('first_name', '') if nppes_result else ''
            provider_first = st.text_input("Provider First Name *", key="provider_first", value=default_first)
            provider_specialty = st.text_input("Specialty (Optional)", key="provider_specialty")
        with col2:
            default_last = nppes_result.get('last_name', '') if nppes_result else ''
            provider_last = st.text_input("Provider Last Name *", key="provider_last", value=default_last)
            provider_middle = st.text_input("Middle Initial (Optional)", key="provider_middle")
        
        col1, col2 = st.columns(2)
        with col1:
            default_phone = nppes_result.get('phone', '') if nppes_result else ''
            provider_phone = st.text_input("Phone (Optional)", key="provider_phone", value=default_phone)
            provider_credentials = st.text_input("Credentials (Optional)", key="provider_credentials")
        with col2:
            default_address = nppes_result.get('address', '') if nppes_result else ''
            provider_address = st.text_input("Address (Optional)", key="provider_address", value=default_address)
        
        col1, col2, col3 = st.columns(3)
        with col1:
            default_city = nppes_result.get('city', '') if nppes_result else ''
            provider_city = st.text_input("City (Optional)", key="provider_city", value=default_city)
        with col2:
            default_state = nppes_result.get('state', '') if nppes_result else ''
            provider_state = st.text_input("State (Optional)", key="provider_state", value=default_state)
        with col3:
            default_zip = nppes_result.get('zip', '') if nppes_result else ''
            provider_zip = st.text_input("ZIP (Optional)", key="provider_zip", value=default_zip)
        ...
        for i in range(4):
            ...
            with col1:
                diag_code = st.text_input(f"Diagnosis {i+1} Code", key=f"diag_code_{i}", placeholder="M54.5")
            with col2:
        ...
        if diagnoses:
            st.caption(f"✓ {len(diagnoses)} diagnosis code(s) entered")
        else:
            st.caption("ⓘ At least one diagnosis code required")
        
        st.divider()
        
        # ===== BOX 24: SERVICE LINES =====
        st.subheader("Box 24: Service Line Details (Repeatable)")
        st.write("Add service lines for this claim (1-10):")
        
        num_lines = st.number_input("Number of Service Lines", min_value=1, max_value=10, value=1, key="num_lines", step=1)
        ...
        for line_num in range(int(num_lines)):
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                from_date = st.date_input(f"From Date *", key=f"from_date_{line_num}", format="YYYY/MM/DD")
            with col2:
                to_date = st.date_input(f"To Date *", key=f"to_date_{line_num}", format="YYYY/MM/DD")
            with col3:
                procedure_code = st.text_input(f"Procedure Code (CPT/HCPCS) *", key=f"proc_code_{line_num}", placeholder="99213")
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                place_of_service = st.text_input(f"Place of Service *", value="11", key=f"pos_{line_num}", placeholder="11=Office")
            with col2:
                units = st.number_input(f"Units", min_value=1, value=1, key=f"units_{line_num}", step=1)
            with col3:
                charges = st.number_input(f"Charges ($) *", min_value=0.0, value=0.0, key=f"charges_{line_num}", step=0.01)
            with col4:
                st.write("")
        ...
        col1, col2 = st.columns(2)
        with col1:
            federal_tax_id = st.text_input("Federal Tax ID *", key="fed_tax_id", placeholder="12-3456789")
        ...
        if submitted:
            ...
            if not patient_first or not patient_last:
                errors.append("Patient name (first and last) required")
            if not provider_npi or not provider_last or not provider_first:
                errors.append("Provider information (NPI, first name, last name) required")
            if not diagnoses:
            ...
            # Validate service lines have data
            for sl in service_lines_data:
                if not sl['procedure_code']:
                    errors.append(f"Service line {sl['line_number']}: procedure code required")
                if sl['charges'] <= 0:
                    errors.append(f"Service line {sl['line_number']}: charges must be > 0")
            
            # Validate diagnoses
            if len(diagnoses) > 4:
                errors.append("Maximum 4 diagnoses allowed")
            
            # Show errors if any
            if errors:
                st.error("❌ Form has errors:\n" + "\n".join(f"• {e}" for e in errors))
                return None
            ...
            service_line_objs = [
                ServiceLine(
                    ...
                    charges=sl['charges'],
                    units=sl['units'],
                    diagnosis_pointer=sl['diagnosis_pointer'],
                    emg=False,
                )
                for sl in service_lines_data
            ]
            ...
            cms1500 = CMS1500(
                ...
                service_lines=service_line_objs,
                diagnoses=diagnosis_objs,
                billing_info=BillingInfo(
                    federal_tax_id=federal_tax_id,
                    federal_tax_id_type=tax_id_type,
                    accept_assignment=accept_assignment,
                    total_charges=sum(sl['charges'] for sl in service_lines_data),
                ),
